...
shape_unet.load_state_dict(torch.load('models/unet_final_nuevo.pth', map_location=device))
shape_unet.eval()
    ...
    if angle is not None:
        if isinstance(angle, np.ndarray):
            if angle.size == 1:
                ...
                print(f"ERROR: Ángulo retornado tiene múltiples valores: {angle}")
                skip = True
        if not skip:
            angle = float(angle)
        ...
        print(f"No se pudo calcular el ángulo del canino {side}")
...
# Guardar salida
nombre_salida = os.path.splitext(os.path.basename(img_path))[0] + '_analizada_nuevo3.jpg'
os.makedirs('resultados', exist_ok=True)
cv2.imwrite(os.path.join('resultados', nombre_salida), orig)
print(f"Resultado guardado en: resultados/{nombre_salida}")